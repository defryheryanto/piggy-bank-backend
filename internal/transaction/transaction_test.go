...
func TestCreateBasic(t *testing.T) {
	...
	test.RunTestWithDB(db, t, func(t *testing.T, db *gorm.DB) {
		...
		t.Run("insert transaction", func(t *testing.T) {
			...
			assert.Equal(t, date.TruncateToDay(payload.TransactionDate), existing.TransactionDate.Local())
			...
			assert.Equal(t, payload.TransactionType, existing.TransactionType)
		})
		...
		t.Run("insert participants", func(t *testing.T) {
			...
			db.Where("transaction_id = ?", existingTrx.TransactionId).First(&existing)
